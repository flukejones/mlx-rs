//! V2 LEAN + fused qsdpa kernel — the recommended **short-context decode**
//! path for quantised KV.
//!
...
//!        falls back to V2 LEAN ops-composed automatically. Model code is
//!        unchanged — `cache.attention(...)` does the right thing.
...
fn main() -> anyhow::Result<()> {
    ...
    eval(&tokens)?;
    println!("decoded {} tokens with V2 LEAN + fused qsdpa", tokens.len());
    Ok(())
}
