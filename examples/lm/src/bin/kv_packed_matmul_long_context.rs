//! Packed-matmul at long context — when the fused kernel falls back.
//!
//! What:  `QuantizedKVCache::with_quantized_matmul()` (no fused-kernel flag).
//! Why:   the fused kernel falls back to ops-composed past `n_k > 4096`
//!        (perf crossover, not a TG-memory limit — mlx's tiled
//!        `quantized_matmul` wins that regime). The packed-matmul path
//!        still beats naive dequant-on-read: at T=8192 it delivers
//!        ~152 tok/s vs ~77 tok/s (+96%).
...
fn main() -> anyhow::Result<()> {
    ...
    // tokeniser produced. T=8192 is where the packed-matmul path's
    // bandwidth win over dequant-on-read is most visible.
    ...
    eval(&tokens)?;
    println!(
        "decoded {} tokens at T={} with packed-matmul (ops-composed at long ctx)",
        tokens.len(),
        LONG_PROMPT_LEN
    );
    Ok(())
