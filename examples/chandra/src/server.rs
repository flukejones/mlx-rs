...
use crate::{AppState, Cli, FinishReason};
...
pub fn run(cli: Cli) -> Result<()> {
    ...
    let rt = tokio::runtime::Builder::new_multi_thread()
        .enable_all()
        .build()?;
    rt.block_on(async move { serve(cli.port, state).await })
}
...
fn extract_user_text(messages: &[ChatMessage]) -> Result<String> {
    let last = messages
        ...
        .rev()
        .find(|m| m.role == "user")
        .ok_or_else(|| anyhow!("no user message in `messages`"))?;
