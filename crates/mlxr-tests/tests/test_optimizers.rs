    ...
    O: Optimizer,
{
    ...
    let mut lg = layers::value_and_grad(loss);
...
where
    O: Optimizer,
{
    use mlxr::optimizers::OptimizerState;
...
#[test]
fn test_ada_delta() {
    mlxr::random::seed(547).unwrap();
    let a = mlxr::random::normal::<f32>(&[4, 3], None, None, None).unwrap();
    ...
    let a_grad = mlxr::random::normal::<f32>(&[4, 3], None, None, None).unwrap();
