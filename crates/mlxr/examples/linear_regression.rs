...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    // True weight vector
    // let w_star = mlxr::random::normal::<f32>(&[num_features], None, None, None)?;
    let w_star = mlxr::normal!(shape = &[num_features])?;

    // Input examples (design matrix)
    // let x = mlxr::random::normal::<f32>(&[num_examples, num_features], None, None, None)?;
    let x = mlxr::normal!(shape = &[num_examples, num_features])?;

    // Noisy labels
    // let eps = mlxr::random::normal::<f32>(&[num_examples], None, None, None)? * 1e-2;
    let eps = mlxr::normal!(shape = &[num_examples])? * 1e-2;
    let y = x.matmul(&w_star)? + eps;

    // Initialize random weights
    // let w = mlxr::random::normal::<f32>(&[num_features], None, None, None)? * 1e-2;
    let w = mlxr::normal!(shape = &[num_features])? * 1e-2;
