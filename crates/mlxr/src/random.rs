...
///
/// ```rust,no_run
/// use mlxr::random::RandomState;
/// use mlxr::transforms::compile::compile_with_state;
/// use mlxr::random::categorical;
/// use mlxr::Array;
///
...
pub fn key(seed: u64) -> Result<Array> {
    Array::try_from_op(|res| unsafe { mlxr_sys::mlx_random_key(res, seed) })
}
...
pub fn split_device(
    ...
    let keys = Array::try_from_op(|res| unsafe {
        mlxr_sys::mlx_random_split_num(res, key.as_ref().as_ptr(), num, stream.as_ref().as_ptr())
    })?;
...
///
/// ```rust
/// let key = mlxr::random::key(0).unwrap();
///
/// // create an array of shape `[50]` type f32 values in the range [0, 10)
/// let array = mlxr::random::uniform::<_, f32>(0, 10, &[50], &key);
///
/// // same, but in range [0.5, 1)
/// let array = mlxr::random::uniform::<_, f32>(0.5f32, 1f32, &[50], &key);
/// ```
...
/// let value = mlxr::random::normal::<f32>(None, None, None, &key).unwrap().item::<f32>();
///
/// // generate an array of f32 with normal distribution in shape [10, 5]
/// let array = mlxr::random::normal::<f32>(&[10, 5], None, None, &key);
/// ```
#[generate_macro(customize(root = "$crate::random"))]
#[default_device]
...
pub fn multivariate_normal_device<'a, T: ArrayElement>(
    ...
    Array::try_from_op(|res| unsafe {
        mlxr_sys::mlx_random_multivariate_normal(
            res,
            mean.as_ref().as_ptr(),
...
pub fn truncated_normal_device<'a, E: Into<Array>, T: ArrayElement>(
    ...
    Array::try_from_op(|res| unsafe {
        mlxr_sys::mlx_random_truncated_normal(
            res,
            lb.as_ptr(),
            ub.as_ptr(),
            shape.as_ptr(),
...
/// let value = mlxr::random::gumbel::<f32>(None, &key).unwrap().item::<f32>();
///
/// // generate an array of Float with Gumbel distribution in shape [10, 5]
/// let array = mlxr::random::gumbel::<f32>(&[10, 5], &key);
/// ```
#[generate_macro(customize(root = "$crate::random"))]
#[default_device]
...
/// let key = mlxr::random::key(0).unwrap();
///
/// let logits = mlxr::Array::zeros::<u32>(&[5, 20]).unwrap();
///
/// // produces Array of u32 shape &[5]
/// let result = mlxr::random::categorical(&logits, None, None, &key);
/// ```
#[generate_macro(customize(root = "$crate::random"))]
#[default_device]
pub fn categorical_device<'a>(
    ...
    match shape_or_count.into() {
        Some(ShapeOrCount::Shape(shape)) => Array::try_from_op(|res| unsafe {
            mlxr_sys::mlx_random_categorical_shape(
                res,
        ...
        Some(ShapeOrCount::Count(num_samples)) => Array::try_from_op(|res| unsafe {
            mlxr_sys::mlx_random_categorical_num_samples(
                res,
                logits.as_ref().as_ptr(),
                axis,
                num_samples,
        ...
        None => Array::try_from_op(|res| unsafe {
            mlxr_sys::mlx_random_categorical(
                res,
                logits.as_ref().as_ptr(),
                axis,
                key.as_ptr(),
