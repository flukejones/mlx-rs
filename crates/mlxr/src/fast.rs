...
pub fn rope_dynamic_device<'a>(
    ...
    let freqs = freqs.into();
    Array::try_from_op(|res| unsafe {
        mlxr_sys::mlx_fast_rope_dynamic(
            res,
            array.as_ref().as_ptr(),
            dimensions,
            traditional,
...
impl ScaledDotProductAttentionMask<'_> {
    fn as_mode_and_mask(&self) -> (&'static CStr, mlxr_sys::mlx_array) {
        match self {
            ScaledDotProductAttentionMask::Array(mask) => (DEFAULT_MASK_MODE, mask.as_ptr()),
            ScaledDotProductAttentionMask::Causal => {
                (CAUSAL_MASK_MODE, unsafe { mlxr_sys::mlx_array_new() })
            }
        }
    }
}
...
pub fn scaled_dot_product_attention_device<'a>(
    ...
    let (mask_mode, mask_arr) = mask.into_option().map_or_else(
        || (DEFAULT_MASK_MODE, unsafe { mlxr_sys::mlx_array_new() }),
        |m| m.as_mode_and_mask(),
    );

    Array::try_from_op(|res| unsafe {
        mlxr_sys::mlx_fast_scaled_dot_product_attention(
            res,
...
impl MetalKernelConfig {
    ...
    fn into_raw(self) -> Result<RawMetalKernelConfig> {
        unsafe {
            let raw = mlxr_sys::mlx_fast_metal_kernel_config_new();
            if raw.ctx.is_null() {
            ...
            for (shape, dtype) in &self.outputs {
                let status = mlxr_sys::mlx_fast_metal_kernel_config_add_output_arg(
                    guard.raw,
                    shape.as_ptr(),
                    shape.len(),
                    u32::from(*dtype),
            ...
            if mlxr_sys::mlx_fast_metal_kernel_config_set_grid(guard.raw, g1, g2, g3) != 0 {
                return Err(Exception::custom("metal_kernel_config: set_grid failed"));
            }

            let (t1, t2, t3) = self.thread_group;
            if mlxr_sys::mlx_fast_metal_kernel_config_set_thread_group(guard.raw, t1, t2, t3) != 0 {
                return Err(Exception::custom(
            ...
            for (name, arg) in &self.template_args {
                let status = match arg {
                    MetalKernelTemplateArg::Dtype(dtype) => {
                        mlxr_sys::mlx_fast_metal_kernel_config_add_template_arg_dtype(
                            guard.raw,
                            name.as_ptr(),
                            u32::from(*dtype),
                        )
                    ...
                    MetalKernelTemplateArg::Int(v) => {
                        mlxr_sys::mlx_fast_metal_kernel_config_add_template_arg_int(
                            guard.raw,
                            name.as_ptr(),
                            *v,
                        )
                    ...
                    MetalKernelTemplateArg::Bool(v) => {
                        mlxr_sys::mlx_fast_metal_kernel_config_add_template_arg_bool(
                            guard.raw,
                            name.as_ptr(),
                            *v,
                        )
            ...
            if let Some(value) = self.init_value {
                if mlxr_sys::mlx_fast_metal_kernel_config_set_init_value(guard.raw, value) != 0 {
                    return Err(Exception::custom(
            ...
            if mlxr_sys::mlx_fast_metal_kernel_config_set_verbose(guard.raw, self.verbose) != 0 {
...
struct RawMetalKernelConfig {
    raw: mlxr_sys::mlx_fast_metal_kernel_config,
}
...
impl Drop for RawMetalKernelConfig {
    fn drop(&mut self) {
        unsafe { mlxr_sys::mlx_fast_metal_kernel_config_free(self.raw) };
    }
}
...
impl MetalKernel {
    ...
    pub fn new(
        ...
        let raw = unsafe {
            mlxr_sys::mlx_fast_metal_kernel_new(
                cname.as_ptr(),
                input_names_vec.raw,
    ...
    pub fn apply(
        ...
        Vec::<Array>::try_from_op(|res| unsafe {
            mlxr_sys::mlx_fast_metal_kernel_apply(
                res,
                self.raw,
                input_vec.as_ptr(),
                raw_config.raw,
...
impl Drop for MetalKernel {
    fn drop(&mut self) {
        unsafe { mlxr_sys::mlx_fast_metal_kernel_free(self.raw) };
    }
}

struct StringVector {
    raw: mlxr_sys::mlx_vector_string,
}
...
impl StringVector {
    fn from_strs(values: &[&str]) -> Result<Self> {
        let cstrs: Vec<CString> = values
            ...
            .collect::<Result<_>>()?;
        let raw = unsafe { mlxr_sys::mlx_vector_string_new() };
        for cs in &cstrs {
            let status = unsafe { mlxr_sys::mlx_vector_string_append_value(raw, cs.as_ptr()) };
            if status != 0 {
                unsafe { mlxr_sys::mlx_vector_string_free(raw) };
...
impl Drop for StringVector {
    fn drop(&mut self) {
        unsafe { mlxr_sys::mlx_vector_string_free(self.raw) };
    }
}
