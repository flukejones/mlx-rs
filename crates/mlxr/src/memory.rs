...
pub fn active_memory() -> usize {
    let mut n: usize = 0;
    unsafe {
        mlxr_sys::mlx_get_active_memory(&mut n);
    }
    n
}
...
pub fn cache_memory() -> usize {
    let mut n: usize = 0;
    unsafe {
        mlxr_sys::mlx_get_cache_memory(&mut n);
    }
    n
}
...
pub fn peak_memory() -> usize {
    let mut n: usize = 0;
    unsafe {
        mlxr_sys::mlx_get_peak_memory(&mut n);
    }
    n
}
...
pub fn reset_peak_memory() {
    unsafe {
        mlxr_sys::mlx_reset_peak_memory();
    }
}
...
pub fn set_cache_limit(limit: usize) -> usize {
    ...
    unsafe {
        mlxr_sys::mlx_set_cache_limit(&mut prev, limit);
    }
    prev
}
...
pub fn set_memory_limit(limit: usize) -> usize {
    let mut prev: usize = 0;
    unsafe {
        mlxr_sys::mlx_set_memory_limit(&mut prev, limit);
    }
    prev
}
