...
impl Drop for MaybeUninitArray {
    fn drop(&mut self) {
        if !self.init_success {
            unsafe {
                mlxr_sys::mlx_array_free(self.ptr);
            }
        }
    }
}
...
pub(crate) struct MaybeUninitVectorArray {
    pub(crate) ptr: mlxr_sys::mlx_vector_array,
    pub(crate) init_success: bool,
}
...
impl MaybeUninitVectorArray {
    pub fn new() -> Self {
        unsafe {
            Self {
                ptr: mlxr_sys::mlx_vector_array_new(),
                init_success: false,
            }
        }
    }
...
impl Drop for MaybeUninitVectorArray {
    fn drop(&mut self) {
        if !self.init_success {
            unsafe {
                mlxr_sys::mlx_vector_array_free(self.ptr);
            }
        }
    }
}
...
impl Guard<Vec<Array>> for MaybeUninitVectorArray {
    type MutRawPtr = *mut mlxr_sys::mlx_vector_array;
    ...
    fn try_into_guarded(mut self) -> Result<Vec<Array>, Exception> {
        ...
        unsafe {
            let size = mlxr_sys::mlx_vector_array_size(self.ptr);
            (0..size)
                .map(|i| Array::try_from_op(|res| mlxr_sys::mlx_vector_array_get(res, self.ptr, i)))
                .collect()
        }
    }
}
...
pub(crate) struct MaybeUninitDevice {
    pub(crate) ptr: mlxr_sys::mlx_device,
    pub(crate) init_success: bool,
}
...
impl Drop for MaybeUninitDevice {
    fn drop(&mut self) {
        if !self.init_success {
            unsafe {
                mlxr_sys::mlx_device_free(self.ptr);
            }
        }
    }
}
...
impl Guard<crate::Device> for MaybeUninitDevice {
    type MutRawPtr = *mut mlxr_sys::mlx_device;
...
impl Guarded for crate::DeviceType {
    type Guard = mlxr_sys::mlx_device_type;
}

impl Guard<crate::DeviceType> for mlxr_sys::mlx_device_type {
...
pub(crate) struct MaybeUninitStream {
    pub(crate) ptr: mlxr_sys::mlx_stream,
    pub(crate) init_success: bool,
}
...
impl Drop for MaybeUninitStream {
    fn drop(&mut self) {
        if !self.init_success {
            unsafe {
                mlxr_sys::mlx_stream_free(self.ptr);
            }
        }
    }
}
...
impl Guard<crate::Stream> for MaybeUninitStream {
    type MutRawPtr = *mut mlxr_sys::mlx_stream;
...
pub(crate) struct MaybeUninitSafeTensors {
    pub(crate) c_data: mlxr_sys::mlx_map_string_to_array,
    pub(crate) c_metadata: mlxr_sys::mlx_map_string_to_string,
    pub(crate) init_success: bool,
}
...
impl MaybeUninitSafeTensors {
    pub fn new() -> Self {
        unsafe {
            Self {
                c_metadata: mlxr_sys::mlx_map_string_to_string_new(),
                c_data: mlxr_sys::mlx_map_string_to_array_new(),
                init_success: false,
            }
        }
    }
...
impl Drop for MaybeUninitSafeTensors {
    fn drop(&mut self) {
        if !self.init_success {
            unsafe {
                mlxr_sys::mlx_map_string_to_string_free(self.c_metadata);
                mlxr_sys::mlx_map_string_to_array_free(self.c_data);
            }
        }
    }
}
...
pub(crate) struct MaybeUninitClosure {
    pub(crate) ptr: mlxr_sys::mlx_closure,
    pub(crate) init_success: bool,
}
...
impl Drop for MaybeUninitClosure {
    fn drop(&mut self) {
        if !self.init_success {
            unsafe {
                mlxr_sys::mlx_closure_free(self.ptr);
            }
        }
    }
}
...
pub(crate) struct MaybeUninitClosureValueAndGrad {
    pub(crate) ptr: mlxr_sys::mlx_closure_value_and_grad,
    pub(crate) init_success: bool,
}
...
impl MaybeUninitClosureValueAndGrad {
    pub fn new() -> Self {
        unsafe {
            Self {
                ptr: mlxr_sys::mlx_closure_value_and_grad_new(),
                init_success: false,
            }
        }
    }
...
impl Drop for MaybeUninitClosureValueAndGrad {
    fn drop(&mut self) {
        if !self.init_success {
            unsafe {
                mlxr_sys::mlx_closure_value_and_grad_free(self.ptr);
            }
        }
    }
}
