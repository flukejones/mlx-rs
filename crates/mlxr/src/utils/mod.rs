...
impl VectorArray {
    ...
    pub(crate) fn try_from_iter(
        ...
        Self::try_from_op(|res| unsafe {
            ...
            for arr in iter {
                status = mlxr_sys::mlx_vector_array_append_value(*res, arr.as_ref().as_ptr());
                if status != SUCCESS {
                    return status;
                }
            }
    ...
    {
        unsafe {
            let size = mlxr_sys::mlx_vector_array_size(self.c_vec);
            (0..size)
                .map(|i| {
    ...
    pub(crate) fn try_into_one(self) -> Result<Array, Exception> {
        ...
        Array::try_from_op(|res| unsafe { mlxr_sys::mlx_vector_array_get(res, self.c_vec, 0) })
    }
    ...
    pub(crate) fn try_into_array<const N: usize>(self) -> Result<[Array; N], Exception> {
        ...
        for (i, slot) in out.iter_mut().enumerate() {
            match Array::try_from_op(|res| unsafe {
                mlxr_sys::mlx_vector_array_get(res, self.c_vec, i)
            }) {
                Ok(arr) => {
                    slot.write(arr);
                }
...
impl Drop for VectorArray {
    fn drop(&mut self) {
        let status = unsafe { mlxr_sys::mlx_vector_array_free(self.c_vec) };
...
pub(crate) struct Closure<'a> {
    c_closure: mlxr_sys::mlx_closure,
...
impl<'a> Closure<'a> {
    pub(crate) fn as_ptr(&self) -> mlxr_sys::mlx_closure {
        self.c_closure
    }
...
impl Drop for Closure<'_> {
    fn drop(&mut self) {
        let status = unsafe { mlxr_sys::mlx_closure_free(self.c_closure) };
        debug_assert_eq!(status, SUCCESS);
    }
}

/// Helper method to create a mlx_closure from a Rust closure.
fn new_mlx_closure<'a, F>(closure: F) -> mlxr_sys::mlx_closure
where
    F: FnMut(&[Array]) -> Vec<Array> + 'a,
{
    ...
    unsafe {
        mlxr_sys::mlx_closure_new_func_payload(
            Some(trampoline::<F>),
            payload,
            Some(closure_dtor::<F>),
        )
...
fn new_mlx_vector_array(arrays: Vec<Array>) -> mlx_vector_array {
    unsafe {
        let result = mlxr_sys::mlx_vector_array_new();
        let ctx_ptrs: Vec<mlxr_sys::mlx_array> =
            arrays.iter().map(|array| array.as_ptr()).collect();
        mlxr_sys::mlx_vector_array_append_data(result, ctx_ptrs.as_ptr(), arrays.len());
        result
    }
}
...
fn mlx_vector_array_values(vector_array: mlx_vector_array) -> Result<Vec<Array>, Exception> {
    unsafe {
        ...
        (0..size)
            .map(|index| {
                Array::try_from_op(|res| mlxr_sys::mlx_vector_array_get(res, vector_array, index))
            })
            .collect()
    }
}
