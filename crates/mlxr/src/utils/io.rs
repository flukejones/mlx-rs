...
impl SafeTensors {
    ...
    pub(crate) fn data(&self) -> Result<HashMap<String, Array>, Exception> {
        ...
        unsafe {
            ...
            loop {
                ...
                let mut value = mlxr_sys::mlx_array_new();
                let status = mlxr_sys::mlx_map_string_to_array_iterator_next(
                    &mut key_ptr,
                    &mut value,
                    iterator,
                );
            ...
            mlxr_sys::mlx_map_string_to_array_iterator_free(iterator);
        }
    ...
    pub(crate) fn metadata(&self) -> Result<HashMap<String, String>, Exception> {
        ...
        let mut map = HashMap::new();
        unsafe {
            let iterator = mlxr_sys::mlx_map_string_to_string_iterator_new(self.c_metadata);

            let mut key: *const ::std::os::raw::c_char = null_mut();
            let mut value: *const ::std::os::raw::c_char = null_mut();
            loop {
                let status = mlxr_sys::mlx_map_string_to_string_iterator_next(
