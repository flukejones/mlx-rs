...
//!
//! ```rust
//! use mlxr::{Array, array, transforms::compile::compile, error::Exception};
//!
//! let fun = |(x, y): (&Array, &Array)| -> Result<Array, Exception> {
//!    mlxr::exp!(x.negative()?)?.add(y)
//! };
//!
//! let x = array!(1.0);
//! let y = array!(2.0);
...
//!
//! ```rust
//! use mlxr::{Array, array, transforms::compile::compile};
//!
...
//!
//! ```rust
//! use mlxr::{Array, array, transforms::compile::compile_with_state};
//! let mut state = vec![];
//!
//! let fun = |state: &mut Vec<Array>, (x, y): (&Array, &Array)| {
//!     let z = x + y;
//!     let result = mlxr::exp!(&z);
//!     state.push(z);
//!     result
//! };
//!
...
pub fn clear_cache() {
    unsafe {
        mlxr_sys::mlx_detail_compile_clear_cache();
    }
}
...
impl<F> Drop for CompiledState<F> {
    fn drop(&mut self) {
        unsafe {
            ...
            mlxr_sys::mlx_detail_compile_erase(self.id);
        }
    }
}
...
fn update_by_replace_with_ref_to_new_array(src: &mut Array, new_array: &Array) {
    ...
    unsafe {
        mlxr_sys::mlx_array_set(&mut src.as_ptr(), new_array.as_ptr());
    }
}
