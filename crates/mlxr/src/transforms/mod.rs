...
//!
//! ```rust,ignore
//! use mlxr::{Array, error::Result, transforms::grad};
//!
...
use mlxr_sys::mlx_closure_value_and_grad;
...
pub fn eval<'a>(outputs: impl IntoIterator<Item = &'a Array>) -> Result<()> {
    ...
    <() as Guarded>::try_from_op(|_| unsafe { mlxr_sys::mlx_eval(vec.as_ptr()) })
}
...
pub fn async_eval<'a>(outputs: impl IntoIterator<Item = &'a Array>) -> Result<()> {
    let vec = VectorArray::try_from_iter(outputs.into_iter())?;
    <() as Guarded>::try_from_op(|_| unsafe { mlxr_sys::mlx_async_eval(vec.as_ptr()) })
}
...
impl Drop for ClosureValueAndGrad {
    fn drop(&mut self) {
        let status =
            unsafe { mlxr_sys::mlx_closure_value_and_grad_free(self.c_closure_value_and_grad) };
...
fn value_and_gradient(
    ...
    <(Vec<Array>, Vec<Array>) as Guarded>::try_from_op(|(res_0, res_1)| unsafe {
        mlxr_sys::mlx_closure_value_and_grad_apply(
            res_0,
            res_1,
            value_and_grad,
            input_vector.as_ptr(),
