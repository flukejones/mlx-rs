...
//!
//! ```rust
//! use mlxr::{array, transforms::eval};
//!
//! let a = array!([1, 2, 3, 4]);
//! let b = array!([1.0, 2.0, 3.0, 4.0]);
//!
...
//!
//! See the [`transforms`] module for `eval`, `compile`, `grad`, `vjp`,
//! `jvp`, and friends.
//!
//! # Lazy Evaluation
//!
...
//!
//! ```rust
//! // let a = mlxr::random::normal(&[100], None, None, None, None).unwrap();
//! // let b = mlxr::random::normal(&[100], None, None, None, None).unwrap();
//!
//! let a = mlxr::normal!(shape=&[100]).unwrap();
//! let b = mlxr::normal!(shape=&[100]).unwrap();
//! ```
//!
...
//!
//! ```rust,ignore
//! // mlxr::ops::add_device(&a, &b, StreamOrDevice::cpu()).unwrap();
//! // mlxr::ops::add_device(&a, &b, StreamOrDevice::gpu()).unwrap();
//!
//! mlxr::add!(&a, &b, stream=StreamOrDevice::cpu()).unwrap();
//! mlxr::add!(&a, &b, stream=StreamOrDevice::gpu()).unwrap();
//! ```
//!
...
// Optional higher-level libraries built on top of the always-compiled
// tensor + module core. Each is independent.
#[cfg(feature = "fast")]
pub mod fast;
#[cfg(feature = "fft")]
pub mod fft;
