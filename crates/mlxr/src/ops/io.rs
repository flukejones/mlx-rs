...
impl Array {
    ...
    pub fn load_numpy_device(
        ...
        Self::try_from_op(|res| unsafe {
            mlxr_sys::mlx_load(res, c_path.as_ptr(), stream.as_ref().as_ptr())
        })
        .map_err(Into::into)
    }
    ...
    pub fn save_numpy(&self, path: impl AsRef<Path>) -> Result<(), IoError> {
        ...
        unsafe { mlxr_sys::mlx_save(c_path.as_ptr(), self.as_ptr()) };
        ...
        V: AsRef<Self>,
    {
        ...
        let arrays = unsafe {
            let data = mlxr_sys::mlx_map_string_to_array_new();
            for (key, array) in arrays {
                let key = CString::new(key.as_ref())?;

                let status = mlxr_sys::mlx_map_string_to_array_insert(
                    data,
                    key.as_ptr(),
                    array.as_ref().as_ptr(),
                );
                ...
                if status != SUCCESS {
                    mlxr_sys::mlx_map_string_to_array_free(data);
        ...
        let metadata = unsafe {
            let data = mlxr_sys::mlx_map_string_to_string_new();
            for (key, value) in metadata_ref {
                let key = CString::new(key.as_str())?;
                let value = CString::new(value.as_str())?;

                let status =
                    mlxr_sys::mlx_map_string_to_string_insert(data, key.as_ptr(), value.as_ptr());

                if status != SUCCESS {
                    mlxr_sys::mlx_map_string_to_string_free(data);
                    return Err(crate::error::get_and_clear_last_mlx_error()
                        .expect("A non-success status was returned, but no error was set.")
                        .into());
                }
        ...
        unsafe {
            let status = mlxr_sys::mlx_save_safetensors(c_path.as_ptr(), arrays, metadata);
            ...
            mlxr_sys::mlx_map_string_to_array_free(arrays);
            mlxr_sys::mlx_map_string_to_string_free(metadata);
