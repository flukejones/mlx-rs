...
/// Derive `mlxr::builder::Buildable` for a struct. When used with the `generate_builder` macro,
...
/// - `root`: Path to the root module. Default to `::mlxr` if not provided.
///
/// ## `#[builder]`
///
...
/// ```rust,ignore
/// use mlxr_codegen::*;
/// use mlxr::builder::{Buildable, Builder};
///
/// generate_builder! {
...
/// Derive `mlxr::builder::Builder` trait for a struct and generate the following methods:
///
...
/// use mlxr::{Stream, StreamOrDevice};
///
