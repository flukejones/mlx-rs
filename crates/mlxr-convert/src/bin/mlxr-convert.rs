...
//! `mlxr_lm::load` (Rust) can read.
//!
//! Drives [`mlxr_convert::convert`] with the right per-family
//! [`mlxr_convert::Rewriter`]. Today only Qwen 3.5 / 3.6 (dense + MoE,
...
use argh::FromArgs;
use mlxr_convert::{convert, qwen3_5::Qwen35Rewriter, ConvertOptions};
...
struct Args {
    ...
    /// after convert, load the destination via `mlxr_lm::load` and run a
...
/// Round-trip the destination dir through `mlxr_lm::load` and run a
...
fn verify_load_and_generate(dst: &std::path::Path) -> anyhow::Result<()> {
    log::info!("verify: loading {} via mlxr_lm::load", dst.display());
    let t = Instant::now();
