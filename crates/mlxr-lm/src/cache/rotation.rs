...
//! Built once on CPU per layer at cache creation; `mlxr::linalg::qr_device`
//! is CPU-only and the cost is sub-millisecond at `d ∈ {64, 128}`.

use mlxr::linalg::qr_device;
use mlxr::ops::indexing::IndexOp;
use mlxr::ops::{expand_dims, sign};
use mlxr::random::{key, normal_device};
