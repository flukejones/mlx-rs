...
use mlxr::macros::ModuleParameters;
use mlxr::module::{ModuleParameters, Param};
use mlxr::ops::indexing::take_axis;
use mlxr::ops::{
    argsort, expand_dims_axes, gather_mm, gather_qmm, quantize, sum_axes, swap_axes, unflatten,
};
use mlxr::quantization::{MaybeQuantized, Quantizable};
use mlxr::Array;
...
impl<A: SwitchActivation> PackedSwitchFfn<A> {
    ...
    pub fn forward(&mut self, x: &Array, indices: &Array) -> Result<Array, Error> {
        ...
        let parts = mlxr::ops::split_sections(&gate_up, &[self.hidden_dims], -1)?;
