...
//! Silicon via `mlxr::fast::metal_kernel`. Targets prefill (n_q > 1) plus
...
/// quantised triples produced by `mlxr::ops::quantize`; same wire
/// format as `mlxr_lm::cache::QuantizedKVCache`.
pub struct QuantAttentionInputs<'a> {
...
/// - `mask` must be `None`
pub fn quant_attention_dispatch(
    kernel: &MetalKernel,
    inputs: QuantAttentionInputs<'_>,
) -> Result<Array> {
    let q_shape = inputs.q.shape();
    if q_shape.len() != 4 {
        return Err(Exception::custom(format!(
