...
impl mlxr::module::ModuleParameters for RopeVariant {
    ...
    fn parameters(&self) -> mlxr::module::ModuleParamRef<'_> {
        match self {
    ...
    fn parameters_mut(&mut self) -> mlxr::module::ModuleParamMut<'_> {
        match self {
    ...
    fn trainable_parameters(&self) -> mlxr::module::ModuleParamRef<'_> {
        match self {
    ...
    Input: Into<layers::RopeInput<'a>>,
{
    ...
    fn training_mode(&mut self, mode: bool) {
        match self {
            Self::Default(rope) => {
                <layers::Rope as Module<layers::RopeInput<'_>>>::training_mode(rope, mode);
            }
            Self::Llama3(rope) => {
                <Llama3Rope as Module<layers::RopeInput<'_>>>::training_mode(rope, mode);
            }
        }
    }
}
...
pub fn initialize_rope(
    ...
    if rope_type == FloatOrStr::Str("default") || rope_type == FloatOrStr::Str("linear") {
        ...
        let rope = layers::RopeBuilder::new(dims)
