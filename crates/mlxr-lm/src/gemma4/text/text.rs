...
use mlxr::module::{Module, ModuleParamMut, ModuleParamRef, ModuleParameters, Param};
use mlxr::ops::clip;
use mlxr::ops::indexing::IndexOp;
use mlxr::ops::{argpartition_axis, expand_dims_axes, unflatten};
use mlxr::quantization::MaybeQuantized;
...
fn build_layer_rope(
    ...
    if rope_type == "proportional" || (rope_type == "default" && rotated_dims < head_dim) {
        ...
        let rope = layers::RopeBuilder::new(head_dim)
...
pub struct Attention {
    ...
    #[param]
    pub k_norm: Option<layers::RmsNorm>,
    #[param]
...
impl Attention {
    pub fn new(args: &TextConfig, layer_idx: i32) -> Result<Self, Error> {
        ...
        let q_proj = layers::LinearBuilder::new(dim, n_heads * head_dim)
            .bias(false)
            .build()?;
        ...
        let (k_proj, v_proj) = if has_kv {
            let k = layers::LinearBuilder::new(dim, n_kv_heads * head_dim)
                .bias(false)
                .build()?;
            let v = if use_k_eq_v {
                None
                ...
                Some(MaybeQuantized::Original(
                    layers::LinearBuilder::new(dim, n_kv_heads * head_dim)
                        .bias(false)
                        .build()?,
                ))
            };
        ...
        let (k_norm, v_norm) = if has_kv {
            let k = layers::RmsNormBuilder::new(head_dim)
                .eps(args.rms_norm_eps)
                .build()?;
    ...
    pub fn new(args: &TextConfig, layer_idx: i32) -> Result<Self, Error> {
        ...
        Ok(Self {
            gate_proj: MaybeQuantized::Original(
                layers::LinearBuilder::new(args.hidden_size, intermediate)
                    .bias(false)
                    .build()?,
            ),
            down_proj: MaybeQuantized::Original(
                layers::LinearBuilder::new(intermediate, args.hidden_size)
                    .bias(false)
                    .build()?,
            ),
...
pub(crate) struct Router {
    ...
    #[quantizable]
    #[param]
    pub proj: MaybeQuantized<layers::Linear>,
    #[param]
    ...
    pub fn new(hidden_size: i32, num_experts: i32, top_k: i32, eps: f32) -> Result<Self, Error> {
        Ok(Self {
            ...
            proj: MaybeQuantized::Original(
                layers::LinearBuilder::new(hidden_size, num_experts)
                    .bias(false)
                    .build()?,
            ),
...
pub struct DecoderLayer {
    ...
    #[param]
    pub pre_feedforward_layernorm: layers::RmsNorm,
    #[param]
    ...
    #[param]
    pub pre_feedforward_layernorm_2: Option<layers::RmsNorm>,
    #[param]
    pub post_feedforward_layernorm_2: Option<layers::RmsNorm>,

    // Per-layer input gating (E2B/E4B). All three fields are `None`
    // when `hidden_size_per_layer_input == 0`.
    #[quantizable]
    #[param]
    pub per_layer_input_gate: Option<MaybeQuantized<layers::Linear>>,
    #[quantizable]
    #[param]
    pub per_layer_projection: Option<MaybeQuantized<layers::Linear>>,
    #[param]
    pub post_per_layer_input_norm: Option<layers::RmsNorm>,
    ...
    pub fn new(args: &TextConfig, layer_idx: i32) -> Result<Self, Error> {
        ...
        let (router, experts, post1, pre2, post2) = if enable_moe {
            ...
            (
                ...
                Some(
                    layers::RmsNormBuilder::new(args.hidden_size)
                        .eps(args.rms_norm_eps)
                        .build()?,
                ),
                Some(
        ...
        let (gate, proj, pl_norm) = if args.hidden_size_per_layer_input > 0 {
            ...
            (
                Some(MaybeQuantized::Original(
                    layers::LinearBuilder::new(args.hidden_size, pl)
                        .bias(false)
                        .build()?,
                )),
                Some(MaybeQuantized::Original(
                    layers::LinearBuilder::new(pl, args.hidden_size)
                        .bias(false)
                        .build()?,
                )),
                Some(
        ...
        Ok(Self {
            ...
            input_layernorm: layers::RmsNormBuilder::new(args.hidden_size)
                .eps(args.rms_norm_eps)
                .build()?,
            post_attention_layernorm: layers::RmsNormBuilder::new(args.hidden_size)
                .eps(args.rms_norm_eps)
                .build()?,
            pre_feedforward_layernorm: layers::RmsNormBuilder::new(args.hidden_size)
                .eps(args.rms_norm_eps)
                .build()?,
            post_feedforward_layernorm: layers::RmsNormBuilder::new(args.hidden_size)
                .eps(args.rms_norm_eps)
                .build()?,
...
pub struct Gemma4TextModel {
    ...
    #[quantizable]
    #[param]
    pub embed_tokens_per_layer: Option<MaybeQuantized<layers::Embedding>>,
    // mlx-community Gemma 4 E2B/E4B keeps this projection unquantised;
    // staying `nn::Linear` avoids inventing missing `.scales`/`.biases`.
    #[param]
    pub per_layer_model_projection: Option<layers::Linear>,
    #[param]
    pub per_layer_projection_norm: Option<layers::RmsNorm>,
}
...
impl Gemma4TextModel {
    pub fn new(args: &TextConfig) -> Result<Self, Error> {
        ...
        let norm = layers::RmsNormBuilder::new(args.hidden_size)
            .eps(args.rms_norm_eps)
            .build()?;
        ...
        let (etpl, plproj, plnorm) = if pl > 0 {
            (
                Some(MaybeQuantized::Original(layers::Embedding::new(
                    args.vocab_size_per_layer_input,
                    args.num_hidden_layers * pl,
                )?)),
                Some(
                    layers::LinearBuilder::new(args.hidden_size, args.num_hidden_layers * pl)
                        .bias(false)
                        .build()?,
                ),
                Some(
                    layers::RmsNormBuilder::new(pl)
                        .eps(args.rms_norm_eps)
                        .build()?,
                ),
            )
...
impl Model {
    pub fn new(args: TextConfig) -> Result<Self, Error> {
        ...
        let lm_head = if args.tie_word_embeddings {
            ...
            Some(MaybeQuantized::Original(
                layers::LinearBuilder::new(args.hidden_size, args.vocab_size)
                    .bias(false)
                    .build()?,
            ))
        };
