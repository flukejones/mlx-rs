...
pub struct PatchMerger {
    ...
    #[param]
    pub norm: layers::LayerNorm,

    #[quantizable]
    #[param]
    pub linear_fc1: MaybeQuantized<layers::Linear>,

    #[quantizable]
    #[param]
    pub linear_fc2: MaybeQuantized<layers::Linear>,
}
...
impl PatchMerger {
    ...
    pub fn new(cfg: &VisionConfig, use_postshuffle_norm: bool) -> Result<Self, Error> {
        ...
        let norm = layers::LayerNormBuilder::new(norm_width)
            .eps(1e-6)
            .build()?;
        let linear_fc1 = layers::LinearBuilder::new(hidden_size, hidden_size).build()?;
        let linear_fc2 = layers::LinearBuilder::new(hidden_size, cfg.out_hidden_size).build()?;
        Ok(Self {
            hidden_size,
...
impl VisionMlp {
    pub fn new(cfg: &VisionConfig) -> Result<Self, Error> {
        ...
        let linear_fc2 =
            layers::LinearBuilder::new(cfg.intermediate_size, cfg.hidden_size).build()?;
        Ok(Self {
            linear_fc1: MaybeQuantized::Original(linear_fc1),
            linear_fc2: MaybeQuantized::Original(linear_fc2),
        })
...
pub struct VisionAttention {
    ...
    #[quantizable]
    #[param]
    pub qkv: MaybeQuantized<layers::Linear>,
    #[quantizable]
    #[param]
...
impl VisionAttention {
    pub fn new(cfg: &VisionConfig) -> Result<Self, Error> {
        ...
        let qkv = layers::LinearBuilder::new(dim, dim * 3).build()?;
        let proj = layers::LinearBuilder::new(dim, dim).build()?;
        Ok(Self {
            num_heads,
            head_dim,
            scale,
...
pub struct VisionBlock {
    #[param]
    pub norm1: layers::LayerNorm,
    #[param]
    pub norm2: layers::LayerNorm,
    #[quantizable]
    #[param]
...
impl VisionBlock {
    pub fn new(cfg: &VisionConfig) -> Result<Self, Error> {
        let norm1 = layers::LayerNormBuilder::new(cfg.hidden_size)
            .eps(1e-6)
            .build()?;
        let norm2 = layers::LayerNormBuilder::new(cfg.hidden_size)
            .eps(1e-6)
            .build()?;
...
pub struct VisionModel {
    ...
    #[quantizable]
    #[param]
    pub pos_embed: MaybeQuantized<layers::Embedding>,
...
impl VisionModel {
    ...
    pub fn new(cfg: &VisionConfig) -> Result<Self, Error> {
        ...
        let pos_embed = layers::Embedding::new(cfg.num_position_embeddings, cfg.hidden_size)?;
