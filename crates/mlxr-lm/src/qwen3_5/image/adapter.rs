...
impl LanguageModel for Qwen35VlmAdapter {
    ...
    fn prepare(&mut self, input: LMInput) -> Result<PrepareResult, Error> {
        ...
        let embeds = match &mut self.dense.model.model.embed_tokens {
            mlxr::quantization::MaybeQuantized::Original(e) => e.forward(&input_ids)?,
            mlxr::quantization::MaybeQuantized::Quantized(q) => q.forward(&input_ids)?,
        };
...
fn concat_patches(arrays: &[Array]) -> Result<Array, Error> {
    ...
    mlxr::ops::concatenate_axis(arrays, 0).map_err(Error::Exception)
}
...
/// Load a qwen3_5 VLM checkpoint at `dir`. Caller guarantees the
/// directory contains a vision tower (i.e. `preprocessor_config.json`
