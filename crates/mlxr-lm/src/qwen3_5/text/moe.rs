...
//! [`super::layer::Qwen35Model<Qwen35MoeBlock>`] gives us the shared
//! decoder skeleton + KV cache + generation iterator.
...
use mlxr::module::{Module, ModuleParameters};
use mlxr::ops::indexing::{take_along_axis, IndexOp};
use mlxr::ops::{argpartition_axis, softmax_axis, sum_axis};
use mlxr::quantization::{MaybeQuantized, Quantizable as _};
...
pub struct Qwen35MoeBlock {
    ...
    #[quantizable]
    #[param]
    pub gate: MaybeQuantized<layers::Linear>,
    ...
    #[quantizable]
    #[param]
    pub shared_expert_gate: MaybeQuantized<layers::Linear>,
...
impl Qwen35MoeBlock {
    ...
    pub fn new(
        ...
        let shared_expert_gate = layers::LinearBuilder::new(hidden_size, 1)
            .bias(false)
            .build()?;
        Ok(Self {
...
/// Concrete alias: the Qwen3.6-MoE model is the qwen3_5 decoder
...
pub use super::cache::{make_caches, LayerCache};
...
fn requantise_linear(
    slot: &mut MaybeQuantized<layers::Linear>,
    group_size: i32,
    bits: i32,
) -> Result<(), Error> {
    let dummy = layers::LinearBuilder::new(1, 1)
        .bias(false)
        .build()
        .map_err(Error::Exception)?;
    ...
    let linear = match old {
        ...
        MaybeQuantized::Quantized(q) => {
            ...
            layers::LinearBuilder::new(in_features, out_features)
                .bias(false)
                .build()
                .map_err(Error::Exception)?
        }
