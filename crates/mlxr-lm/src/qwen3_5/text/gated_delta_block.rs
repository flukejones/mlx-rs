...
// See `mlxr_lm::activations` for the rationale on the fn-item-to-fn-pointer
...
fn precise_swiglu_inner((h, gate, x): (&Array, &Array, &Array)) -> Result<Array, Exception> {
    let gate_silu = layers::silu(gate.as_dtype(Dtype::Float32)?)?;
...
pub struct GatedDeltaNet {
    ...
    #[param]
    pub conv1d: layers::Conv1d,

    /// `Linear(hidden, key_dim*2 + value_dim)`.
    #[quantizable]
    #[param]
    pub in_proj_qkv: MaybeQuantized<layers::Linear>,
    /// `Linear(hidden, value_dim)`.
    #[quantizable]
    #[param]
    pub in_proj_z: MaybeQuantized<layers::Linear>,
    /// `Linear(hidden, num_v_heads)`.
    #[quantizable]
    #[param]
    pub in_proj_b: MaybeQuantized<layers::Linear>,
    /// `Linear(hidden, num_v_heads)`.
    #[quantizable]
    #[param]
    pub in_proj_a: MaybeQuantized<layers::Linear>,
    ...
    #[quantizable]
    #[param]
    pub out_proj: MaybeQuantized<layers::Linear>,
...
impl GatedDeltaNet {
    ...
    pub fn new(cfg: &TextConfig) -> Result<Self, Exception> {
        ...
        let in_proj_qkv = layers::LinearBuilder::new(hidden, conv_dim)
            .bias(false)
            .build()?;
        let in_proj_z = layers::LinearBuilder::new(hidden, value_dim)
            .bias(false)
            .build()?;
        let in_proj_b = layers::LinearBuilder::new(hidden, num_v_heads)
            .bias(false)
            .build()?;
        let in_proj_a = layers::LinearBuilder::new(hidden, num_v_heads)
            .bias(false)
            .build()?;
        let out_proj = layers::LinearBuilder::new(value_dim, hidden)
            .bias(false)
            .build()?;
...
fn depthwise_conv1d(conv_dim: i32, kernel_size: i32) -> Result<layers::Conv1d, Exception> {
