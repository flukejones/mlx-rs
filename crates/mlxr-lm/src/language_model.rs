...
pub trait LanguageModel: Send {
    ...
    fn step(&mut self, last_token: &mlxr::Array) -> Result<LMOutput, Error>;
    ...
    fn prefill_chunk(&mut self, _tokens: &mlxr::Array) -> Result<(), Error> {
        Err(Error::Other(
...
impl UserInputProcessor for TextOnlyProcessor {
    ...
    fn prepare(&mut self, input: UserInput) -> Result<LMInput, Error> {
        ...
        let tokens = mlxr::Array::from_slice(&ids, &[1, len]);
        Ok(LMInput {
