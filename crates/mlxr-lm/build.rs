...
//! `KERNEL_HEADER` for `mlxr::fast::metal_kernel` to compile against.
...
// Identical block in `crates/mlxr-sys/build.rs`. Keep them in sync when edited.

/// Resolve the upstream mlx source tree, fetching it into a shared
/// `target/mlxr-cache/<sha>/mlx/` if absent. Honours the
/// `MLX_RS_SRC_DIR` environment override (e.g. for air-gapped builds
/// or local mlx forks).
///
/// Cargo runs build scripts in parallel, so both `mlxr-sys` and `mlxr-lm`
...
fn ensure_mlx_src() -> PathBuf {
    ...
    let cache_root = workspace_root().join("target/mlxr-cache").join(&sha);
