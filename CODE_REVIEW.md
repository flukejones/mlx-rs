...
- **Flag new `pub` fields on Cargo-exposed structs.** `GenerateParams`, `SamplingParams`, `ModelContext` — adding a field breaks struct-update syntax for consumers using `..Default::default()` only if the field is `pub` without `#[non_exhaustive]`.
