...
fn parity_case_inner(
    ...
    let kernel = make_steel_attention_kernel()?;
    ...
    let steel = steel_attention_dispatch(
        &kernel,
...
fn quant_case(
    ...
    let kernel = make_steel_quant_attention_kernel()?;
    ...
    let scale = 1.0 / (d as f32).sqrt();
    let routed = steel_quant_attention_dispatch(
        &kernel,
        SteelQuantAttentionInputs {
            q: &q,
            k_wq: &k_wq,
