//! Jinja chat-template rendering for `mlx-lm` checkpoints.
//!
//! Mirrors the Hugging Face `tokenizer.apply_chat_template` API: load
//! `chat_template.jinja` (preferred) or `tokenizer_config.json::chat_template`
//! (fallback) from a checkpoint dir, then render a list of `ChatMessage`s
//! through it with `add_generation_prompt = true`. Returns the chat-rendered
//! prompt string ready to feed to the tokenizer.
//!
//! `MessageContent` covers both the simple-string form Llama-family
//! templates expect and the parts-list form Qwen3-VL / Qwen3.5 use for
//! multimodal prompts.
...
impl ChatMessage {
    ...
    /// Build a user message with a single image placeholder followed by text.
    /// The vision model is expected to splice the image into the
    /// `<|image_pad|>` slot at runtime.
...
/// Message content can be a plain string or a list of typed parts.
///
/// Llama-style templates require strings. Qwen3-VL / Qwen3.5 templates
/// accept lists when the message includes an image or a tool call.
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub enum ContentPart {
    ...
    /// Image placeholder. The template emits the model's image-pad token(s);
...
impl ChatTemplate {
    ...
    ///
    /// `add_generation_prompt` should be `true` for inference (appends the
    /// assistant turn-start marker so the model continues from there) and
    /// `false` for fine-tuning data prep.
    pub fn render(
        &self,
