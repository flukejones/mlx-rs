...
mod tests {
    ...
    /// Reference: do gather_qmm + (weights * y).sum(-2) the "slow" way.
    fn reference(
        activated: &Array, // [T, K, H]
        weights: &Array,   // [T, K]
        w_full: &Array,    // [E, D, H] dequantised (or original f32)
        wq: &Array,        // [E, D, H/pack]
        scales: &Array,
        biases: &Array,
        indices: &Array,
        ...
        let _ = w_full;
        // gather_qmm expects `x` shape `[..., 1, 1, H]` after the inner
        // expansions. Use the same expand_dims trick as SwitchGLU.
        let act_exp = expand_dims_axes(activated, &[-2]).unwrap(); // [T, K, 1, H]
        let y = gather_qmm(
            &act_exp,
            wq,
            scales,
        ...
        .unwrap();
        // y: [T, K, 1, D] → squeeze to [T, K, D]
        let y = y.squeeze_axes(&[-2]).unwrap();
        let weights_exp = expand_dims_axes(weights, &[-1]).unwrap(); // [T, K, 1]
    ...
    fn matches_gather_qmm_path_f32_tiny() {
        ...
        let reference = reference(
            &activated, &weights, &w_full, &wq, &scales, &biases, &indices, group_size, bits,
        );
