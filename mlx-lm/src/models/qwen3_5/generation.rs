...
// sample_with}` or import directly from `mlx_lm::sampler`.
pub use crate::sampler::{sample_with as sample_with_canonical, SamplingParams};
...
// `sample` and `top_p_sample` lifted to `crate::sampler` (Plan 3.3).
// Callers within this file use `sample_with_canonical` (re-exported
// above as `sample_with`).
fn sample(logits: &Array, params: &SamplingParams) -> Result<Array, Exception> {
    crate::sampler::sample_with(logits, params)
}
...
impl<'a> Generate<'a> {
    ...
    /// Caller-owned caches for multi-turn use. Seeds `cursor` from the
    ...
    pub fn with_caches(
        ...
        let cursor = caches.first().map(|c| c.offset()).unwrap_or(0);
        Self {
            model,
            caches,
            stop,
    ...
    /// Consume the generator and return its caches.
    pub fn into_caches(self) -> Vec<LayerCache> {
        self.caches
    }
}
