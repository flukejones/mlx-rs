...
impl Attention {
    ...
    pub fn forward(
        ...
        // Prefer the array mask supplied by the caller. When none is given,
        // fall back to the implicit causal mask when this is a prefill step
        // with more than one token (decode is single-token and needs no mask).
...
///
/// This mirrors `mlx_lm::utils::create_attention_mask` but specialises the
/// cache type to [`ConcatKeyValueCache`] so callers don't need a generic.
pub fn full_attention_mask(
    h: &Array,
    cache: &[Option<ConcatKeyValueCache>],
) -> Result<Option<Array>, Exception> {
    match create_attention_mask(h, cache, Some(true))? {
        Some(AttentionMask::Array(a)) => Ok(Some(a)),
        Some(AttentionMask::Causal) => Err(Exception::custom(
            "full_attention_mask: only Array masks supported",
        )),
        None => Ok(None),
    }
}
