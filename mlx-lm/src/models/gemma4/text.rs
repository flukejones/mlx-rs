...
//!
//! Supports: sliding+global attention with per-layer head_dim (256/512),
//! partial RoPE (proportional), K-eq-V projection, KV sharing across the
//! last `num_kv_shared_layers` layers, final logit softcapping, fp16-safe
//! `clip_residual`.
//!
//! Does NOT yet support: MoE expert routing (26B-A4B), per-layer input
//! embeddings (E2B/E4B). These configs are rejected at `Model::new`.
...
impl Attention {
    ...
    pub fn attend<C: KeyValueCache + Default>(
        ...
        let h = if let Some(cache) = cache.as_mut() {
            cache.attention(&queries, keys.clone(), values.clone(), self.scale, mask)?
        } else {
