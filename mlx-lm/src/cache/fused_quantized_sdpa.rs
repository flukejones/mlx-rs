...
const KERNEL_SOURCE: &str = r#"
    ...
    for (uint k_idx = simd_gid; k_idx < nk; k_idx += BN) {
        ...
        if (uint(BITS) == 4u) {
            ...
            float sum_q = q_pre[0] * 16.0f + q_pre[1] * 256.0f
                        + q_pre[2] * 4096.0f + q_pre[3] * 65536.0f;
            // Reverse the q pre-scaling for sum_q: x_pre[i] was x/c_i, so
            // sum_q here recovers sum_x = sum_pre * c_i. For 4-bit:
            // x_pre[0] = x[0]; x_pre[1] = x[1]/16; x_pre[2] = x[2]/256;
            // x_pre[3] = x[3]/4096. Then sum_x = x_pre[0] + x_pre[1]*16 +
            // x_pre[2]*256 + x_pre[3]*4096. But the line above multiplies
            // by 16/256/... — fix:
            sum_q = q_pre[0] + q_pre[1] * 16.0f
