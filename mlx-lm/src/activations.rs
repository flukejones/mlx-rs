...
//! `transforms::compile`-fused inner kernel in a caller-owned cache so
//! every decoder layer per token reuses one fused graph instead of
//! rebuilding it per call. The cache lives on the owning module struct
//! (e.g. `Mlp::swiglu_cache`) and is borrowed `&mut` per call; mlx core's
...
/// `silu(gate) * x` — Python's `@partial(mx.compile, shapeless=True)
/// swiglu` from `mlx_lm.models.activations`. Caller passes a `&mut
/// SwigluCache` owned by the surrounding module; the compiled graph is
/// built on first call and reused thereafter.
...
/// `gelu_approx(gate) * up` — Gemma 4's `geglu` from
/// `mlx_lm.models.gemma4_text`. Caller passes a `&mut GegluCache`
/// owned by the surrounding module; the compiled graph is built on
