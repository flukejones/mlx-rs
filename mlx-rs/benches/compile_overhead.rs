//! Per-call overhead of `transforms::compile::compile`.
//!
//! The Qwen3.5 hot path issues many tiny clusters of ops (e.g. `compute_g`'s
//! 5-op chain on `[Hv]=32` arrays) 24+ times per token. Wrapping such a
//! cluster in `compile(_, Some(true))` regressed throughput in practice;
//! this bench measures whether the regression is dominated by Rust-side
//! per-call closure construction or by the underlying compute.
//!
//! Cases compared:
//!  - `ops_inline`: the cluster called directly, no compile.
//!  - `compile_per_call`: `compile(f, Some(true))` constructed and invoked
//!    once per measurement iteration (current public API).
//!  - `compile_warm`: the closure returned by `compile()` is constructed
//!    once, then invoked many times — exposes whether the FFI / Closure
//!    allocation is per-invocation or per-construction.
...
fn bench_compile_overhead(c: &mut Criterion) {
    // Match chandra's GDN per-layer shape: a_log/dt_bias = [Hv], a = [B,T,Hv].
    let hv = 32;
